...
from loading import load_transaction_data, load_votes_data, load_features_voters_data, \
    load_transaction_data_hug, load_votes_data_hug, load_features_voters_data_hug
...
def display():
    ...
    # st.table(df_fv.loc[df_fv['has_lcs'], ['address', 'has_lcs', 'cluster_size_lcs', 'lcs']])
...
if local:
    ...
    data_load_state = st.text('Loading data...')
    df_tx = load_transaction_data_hug()
    df_v = load_votes_data_hug()
    # df_p = load_projects_data_hug()
    df_fv = load_features_voters_data_hug()
    display()
else:
